...
These were two interesting challenges to get it. While the node takes care of a chunk of setup, it fails to take
care of all the blinking / bias movement (bias of the blink between the upperLid and lowerLid positions)

So really the entire setup needed to be built in such a way that allowed me to consider those two ON TOP of the
node setup.
